...
class TestStringMethods(unittest.TestCase):
    ...
    def test_001_adiciona_alunos(self):
        r = requests.post('http://localhost:5000/alunos',json={'nome':'Andrey_Gabryel','id':1})
        r = requests.post('http://localhost:5000/alunos',json={'nome':'Henry_Cleffs','id':2})
        r_lista = requests.get('http://localhost:5000/alunos')
        lista_retornada = r_lista.json()
        achei_Andrey_Gabryel = False
        achei_Henry_Cleffs = False
        for aluno in lista_retornada:
            if aluno['nome'] == 'Andrey_Gabryel':
                achei_Andrey_Gabryel = True
            if aluno['nome'] == 'Henry_Cleffs':
                achei_Henry_Cleffs = True
        if not achei_Andrey_Gabryel:
            self.fail('aluno Andrey_Gabryel nao apareceu na lista de alunos')
        if not achei_Henry_Cleffs:
            self.fail('aluno Henry_Cleffs nao apareceu na lista de alunos')
    ...
    def test_101_adiciona_professores(self):
        r = requests.post('http://localhost:5000/professores',json={'nome':'Andrey_Gabryel','id':1})
        r = requests.post('http://localhost:5000/professores',json={'nome':'Henry_Cleffs','id':2})
        r_lista = requests.get('http://localhost:5000/professores')
        achei_Andrey_Gabryel = False
        achei_Henry_Cleffs = False
        for professor in r_lista.json():
            if professor['nome'] == 'Andrey_Gabryel':
                achei_Andrey_Gabryel = True
            if professor['nome'] == 'Henry_Cleffs':
                achei_Henry_Cleffs = True
        if not achei_Andrey_Gabryel:
            self.fail('professor Andrey_Gabryel nao apareceu na lista de professores')
        if not achei_Henry_Cleffs:
            self.fail('professor Henry_Cleffs nao apareceu na lista de professores')
    ...
    def test_110_adiciona_turma_a_professor(self):
        r_reset = requests.post('http://localhost:5000/reseta')
        self.assertEqual(r_reset.status_code, 200)
        r = requests.post('http://localhost:5000/professores', json={'nome': 'Carlos', 'id': 3})
        self.assertEqual(r.status_code, 201)
        r = requests.post('http://localhost:5000/turmas', json={'descricao': 'Turma A', 'professor_id': 3, 'ativo': True})
        self.assertEqual(r.status_code, 201)
        r = requests.get('http://localhost:5000/turmas')
        turmas = r.json()
        self.assertEqual(len(turmas), 1)
        self.assertEqual(turmas[0]['professor_id'], 3)
    
    def test_111_relacao_aluno_turma(self):
        r_reset = requests.post('http://localhost:5000/reseta')
        self.assertEqual(r_reset.status_code, 200)
        r = requests.post('http://localhost:5000/turmas', json={'descricao': 'Turma F', 'professor_id': 8, 'ativo': True})
        self.assertEqual(r.status_code, 201)
        r = requests.post('http://localhost:5000/alunos', json={'nome': 'Fernanda', 'id': 10, 'turma_id': 1})
        self.assertEqual(r.status_code, 201)
        r = requests.get('http://localhost:5000/alunos/10')
        aluno = r.json()
